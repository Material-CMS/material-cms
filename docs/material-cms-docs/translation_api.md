...
- ✅ Removed `exposeTranslations` option (API-only delivery)
- ✅ Removed `base64Encode` option (no HTML exposure needed)
- ✅ Eliminated global translation map from HTML
- ✅ Updated language switcher to use API-first approach
- ✅ Implemented deterministic content-based translation IDs

### Remaining Legacy Artifacts
1. **`translationsMap` template variable** - Still exposed in template context but unused
2. **Commented DeepL configuration** - In `data/local.js` (should be removed)
3. **`local.example.js` deprecated options** - Contains legacy `exposeTranslations` documentation
...
### Common Issues
1. **No translations updated when switching language**
   - Check browser console for API errors
   - Verify `data-translation-id` attributes exist on elements
   - Confirm API endpoint is accessible

2. **Translation IDs not matching**
   - Ensure consistent content hashing (check for whitespace differences)
   - Verify the same translation object is being hashed

3. **DeepL integration not working**
   - Check `DEEPL_API_KEY` environment variable
   - Verify module is registered in `app.js`
