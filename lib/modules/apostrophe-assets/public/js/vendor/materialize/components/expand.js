  ...
  var methods = {
    ...
    init : function(options) {
      ...
      return this.each(function(i) {
        ...
        var $object = origin.find('.expand-cover');
        var $header = origin.find('.expand-header');
        var $curveWrapper = origin.find('.expand-curve-wrapper');
        var $body = origin.find('.expand-body');
        var $container = origin.find('.expand-container');
        var $action = origin.find('.expand-action');
        ...
        var isHorizontal = $(this).hasClass('expand-horizontal');
        ...
        var origObjectWidth, origObjectHeight, origObjectRect, origContainerRect;
        var origScrollTop;
        ...
        if (!$header.length) {
          $header = $('<div class="expand-header invisible"></div>');
          if ($object.length) {
            $object.after($header);
          } else {
        ...
        var resetSelectors = function() {
          $object = origin.find('.expand-cover');
          $header = origin.find('.expand-header');
          $curveWrapper = origin.find('.expand-curve-wrapper');
          $body = origin.find('.expand-body');
          $action = origin.find('.expand-action');
        ...
        origin.off('click.expand').on('click.expand', function(e) {
          ...
          if ($object.length) {
            ...
            // Get expand-cover height divide it by 2 and set it as top-margin
            $container.css({
              marginTop: origObjectHeight / 2
            });
          }
          ...
          // add radius to expand-cover
          $object.addClass('radius');
          ...
          var apos = $('.apos-area[data-dot-path="sectionArea"]');
          if (!overlay.length) {
          ...
          overlay
            ...
            .on('click.expand', function() {
              ...
              // Remove Apos no-hover
              apos.removeClass('no-hover');

            })
          if (fillScreen) {
            overlay
              .off('mouseenter.expand')
              .off('mouseleave.expand')
              .on('mouseenter.expand', function() {
                $object.addClass('hover');
              })
              .on('mouseleave.expand', function() {
                $object.removeClass('hover');
              });
          }
          setTimeout(function() {
            overlay.addClass('visible');
            apos.addClass('no-hover');
          }, 0);

          // Add Navbar
          var navbar = $('<nav id="placeholder-navbar"></nav>');
          var navWrapper = $('<div class="nav-wrapper nav-color"></div>');
          var container = $('<div class="nav-container"></div>');
          var backBtn = $('<button class="back-btn text-color flow-text"><i class="material-icons">arrow_back</i><span>Back</span></button>');
          var originalNavColor = '' || options.navOriginalColor;;
          if ($('nav').length) {
        ...
        function returnToOriginal() {
          ...
          $object
            .css('left', animationEndState.find('.expand-cover').css('left'))
          ...
          setTimeout(function() {
            ...
            origin.find('.expand-body').css('display', 'none');
            if ($action) {
            ...
            animationTimeout = setTimeout(function() {
              ...
              // remove radius to from-cover
              $object.removeClass('radius');
    ...
    // Custom methods.
    open : function() {
      $(this).trigger('click.expand');
    },
    close : function() {
      var overlay = $('#placeholder-overlay');
      if (overlay.length) {
        overlay.trigger('click.expand');
      }
    },
  };

    $.fn.expand = function(methodOrOptions) {
      if ( methods[methodOrOptions] ) {
        ...
        $.error( 'Method ' +  methodOrOptions + ' does not exist on jQuery.expand' );
      }
    }; // Plugin end
}( jQuery, M.anime ));
