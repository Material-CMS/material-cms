apos.utils.widgetPlayers['card'] = function(el, data, options) {
  ...
  if (data.clickAction == 'clickActionAnime') {
    ...
    function imageAnimationOn() {
      ...
      cardContent.style.paddingTop = '6rem';
      sectionHeader.style.opacity = '0';
      overlay.style.cssText = 'display: block; opacity: 1;';
      animeImage.classList.add('active');

      for (var i = 0; i < aposUiElements.length; i++) {
        // console.log(aposUiElements[i]);
        // aposUiElements[i].style.display = 'none';
      }
      ...
      var imageAnimation = anime({
        ...
        scale: 0.5,
        translateX: '-40vw',
        translateY: '-40vh', // TODO: calculate actual image height for portrait images, just works for landscape right now
        duration: 500,
