...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Optional: Add server-side API for markdown sanitization
    self.route('post', 'sanitize', function(req, res) {
      var html = req.body.html;
      if (!html || typeof html !== 'string') {
        return res.send({ status: 'error', message: 'Invalid HTML' });
      }
      var sanitized = sanitizeHtml(html);
      res.send({ status: 'ok', html: sanitized });
    });
  }
};