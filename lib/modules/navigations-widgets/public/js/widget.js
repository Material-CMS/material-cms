apos.utils.widgetPlayers['navigations'] = function(el, data, options) {
  ...
  // Go back function to differentiate the go back function based on visited pages
  // Set the last internal page when navigating away
  ...
  function navigateBackOrHome() {
    var lastInternalPage = sessionStorage.getItem("lastInternalPage");
    var currentPage = window.location.href;
    if (lastInternalPage && lastInternalPage !== currentPage) {
  ...
  // Get go back buttons and add eventListener
  var goBackButtons = el.getElementsByClassName('go-back');
  for (var i = 0; i < goBackButtons.length; i++) {
    var goBackButton = goBackButtons[i];
    goBackButton.addEventListener('click', function(e) {
      e.preventDefault();
      navigateBackOrHome(this);
    });
  }
};
