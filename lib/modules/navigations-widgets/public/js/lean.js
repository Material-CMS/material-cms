apos.utils.widgetPlayers['navigations'] = function(el, data, options) {
  ...
  // Activate grandParent of dropdown-content
  function activateGrandParent(element) {
    return element.parentElement.parentElement.classList.add('active') || "unknown";
  }

  // Check if an active li element exists inside dropdown-content
  var ChildUrlActive = el.querySelector('.dropdown-content li.active');
  if (ChildUrlActive) {
    activateGrandParent(ChildUrlActive);
  }
