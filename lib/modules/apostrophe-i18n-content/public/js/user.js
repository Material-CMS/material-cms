apos.define('apostrophe-i18n-content', {
  afterConstruct: function(self) {
    console.log('apostrophe-i18n-content: afterConstruct (browser)');
    self.addFieldType();
  },
  construct: function(self, options) {
    ...
    console.log('apostrophe-i18n-content module constructed, locales:', self.locales);
    
    self.addFieldType = function() {
      console.log('apostrophe-i18n-content: Registering field type multilingual-string (browser)');
      apos.schemas.addFieldType({
        name: 'multilingual-string',
        populate: self.populate,
        convert: self.convert
      });
      console.log('apostrophe-i18n-content: Field type registered');
    };
    
    // Populate the field when the modal opens
    self.populate = function(object, name, $field, $el, field, callback) {
      console.log('Populating multilingual-string field', name);
      ...
      } else if (currentValue && currentValue._translations && typeof currentValue._translations === 'object') {
        translations = currentValue._translations;
    ...
    self.convert = function(object, name, $field, $el, field, callback) {
      console.log('Converting multilingual-string field', name);
      var $fieldset = apos.schemas.findFieldset($el, name);
      ...
      } else if (parsed._translations && typeof parsed._translations === 'object') {
        translations = parsed._translations;
      }
      ...
      // Store both properties for compatibility
      object[name] = {
