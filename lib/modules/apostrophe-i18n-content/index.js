...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    // Request-specific translation collection (legacy, kept for compatibility)
    ...
    // Store translation in MongoDB (idempotent upsert)
    self.storeTranslation = function(id, translations, callback) {
      ...
      self.translationsCollection.updateOne(
        { id: id },
        { $set: { id: id, translations: translations, updatedAt: new Date() } },
        { upsert: true },
        callback
      );
    };
    ...
    self.fetchTranslation = function(id, locale, callback) {
      ...
      self.translationsCollection.findOne({ id: id }, function(err, doc) {
        ...
        if (!doc || !doc.translations || !doc.translations[locale]) {
          return callback(null, null);
        }
        callback(null, doc.translations[locale]);
      });
    };
    ...
    self.translationsAttributeFilter = function(value) {
      ...
      // Store translations for API delivery
      var req = self.apos.templates.contextReq;
      if (req) {
        var reqTranslations = self.getRequestTranslations(req);
        reqTranslations[id] = translations;
        
        // Also store in MongoDB asynchronously (fire-and-forget)
        if (self.translationsCollection) {
    ...
    self.route('get', 'translations', function(req, res) {
      ...
      // Fetch translations from request-specific storage (legacy) and MongoDB
      var reqTranslations = self.getRequestTranslations(req);
      ...
      ids.forEach(function(id) {
        ...
        self.fetchTranslation(id, locale, function(err, translation) {
          ...
          if (translation) {
            ...
            // Fallback to request-specific storage
