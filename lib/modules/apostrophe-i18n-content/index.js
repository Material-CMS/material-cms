...
module.exports = {
  ...
  afterConstruct: function(self) {
    ...
    self.ensureTranslationCollection(function(err) {
      if (err) {
        console.error('Failed to ensure translation collection:', err);
      }
    });
  },
  ...
  construct: function(self, options) {
    ...
    // LRU cache for translation documents
    self.translationCache = new LRUCache({ max: 1000 });

    // MongoDB collection for persistent translation storage
    self.ensureTranslationCollection = function(callback) {
      // If no callback provided, use a dummy
      var cb = callback || function(err) {
        if (err) {
          console.warn('Error ensuring translation collection:', err);
        }
      };
      self.apos.db.collection('aposI18nTranslations', function(err, collection) {
        if (err) return cb(err);
        self.translationsCollection = collection;
        // Ensure unique index on id
        collection.createIndex({ id: 1 }, { unique: true }, function(err) {
          if (err) {
            console.warn('Could not create index on translations collection:', err);
          }
          return cb(null);
        });
      });
    ...
    // Store translation in MongoDB (idempotent upsert) and update cache
    self.storeTranslation = function(id, translations, callback) {
      if (!self.translationsCollection) {
        // Collection not ready yet, skip (should not happen)
        return callback(null);
      }
      var doc = { id: id, translations: translations, updatedAt: new Date() };
      self.translationsCollection.updateOne(
        { id: id },
        { $set: doc },
        { upsert: true },
        function(err, result) {
          if (err) return callback(err);
          // Update cache with the document
          self.translationCache.set(id, doc);
          callback(null, result);
        }
      );
    };

    // Fetch translation from MongoDB with LRU cache
    self.fetchTranslation = function(id, locale, callback) {
      if (!self.translationsCollection) {
        return callback(null, null);
      }
      // Check cache first
      var cached = self.translationCache.get(id);
      if (cached) {
        var translation = cached.translations && cached.translations[locale];
        return callback(null, translation || null);
      }
      // Not in cache, query MongoDB
      self.translationsCollection.findOne({ id: id }, function(err, doc) {
        if (err) return callback(err);
        if (!doc || !doc.translations) {
          return callback(null, null);
        }
        // Store whole document in cache
        self.translationCache.set(id, doc);
        var translation = doc.translations[locale];
        callback(null, translation || null);
      });
    };
    ...
    self.translationsAttributeFilter = function(value) {
      ...
      // Store translations in MongoDB asynchronously (fire-and-forget)
      var req = self.apos.templates.contextReq;
      if (req && self.translationsCollection) {
        setImmediate(function() {
          self.storeTranslation(id, translations, function(err) {
            if (err) {
              console.warn('Failed to store translation in MongoDB:', err);
            }
          });
        });
      }
    ...
    self.route('get', 'translations', function(req, res) {
      ...
      // Process each ID asynchronously, then send response
      var processed = 0;
      ...
      ids.forEach(function(id) {
        // Fetch translation from MongoDB
        self.fetchTranslation(id, locale, function(err, translation) {
          if (err) {
            console.warn('Error fetching translation from MongoDB:', err);
          }
          if (translation) {
